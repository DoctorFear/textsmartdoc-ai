...
def render_chat_history():
    for message in st.session_state.messages:
        ...
        if ( message.get("citations") and message.get("mode") != "rag+corag" ): 
            with st.expander("Xem nguồn trích dẫn (Citations) & Highlight"): 
...
def handle_query(prompt, settings, save_current_session_fn, create_new_chat_session_fn):
    ...
    retrieved_docs = []
    self_rag_meta  = None
    logger.info(f"Mode: '{combined_mode}...'") 
    ...
    with st.chat_message("assistant"):
            ...
            try:
                ...
                if combined_mode == "rag+corag": 
                    ...
                    retriever, st.session_state = _update_bm25_cache( 
                        st.session_state, all_docs, top_k,
                    ...
                    if combined_mode == "rag":
                        ...
                        if self_rag_enabled: 
                            ...
                            logger.info(f"Self-RAG | attempts={result['attempts']} | " f"confidence={result['confidence']} | " f"bm25_weight={bm25_weight},faiss_weight={faiss_weight}") 
                ...
                if combined_mode == "corag":
                    st.caption("Chế độ: CoRAG") 
                elif combined_mode == "rag+corag":
                    st.caption("Chế độ: RAG + CoRAG") 
                else:
                    st.caption("Chế độ: RAG") 
        ...
        save_current_session_fn() 
        streamlit_js_eval( js_expressions="parent.document.querySelectorAll('*').forEach(el => el.scrollTop = el.scrollHeight);", key=f"scroll_{int(time.time() * 100000)}" )
